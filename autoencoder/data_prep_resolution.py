...
IMG_SIZE = (64, 64)
LOW_RES_SIZE = (16, 16)   # lower resolution before scaling back up
BATCH_SIZE = 128
SEED = 67
...
# 1. resize down to LOW_RES_SIZE
# 2. resize back to IMG_SIZE
def get_transforms(img_size=(64, 64), low_res_size=(16, 16)):
    return transforms.Compose([
        transforms.Grayscale(num_output_channels=1),
        transforms.Resize(low_res_size, interpolation=InterpolationMode.BILINEAR),
...
# Load dataset
def load_preprocessed_datasets(data_dir, img_size=(64, 64), low_res_size=(16, 16), batch_size=128, seed=67):
    set_seed(seed)

    data_dir = Path(data_dir)
    transform = get_transforms(img_size, low_res_size)
